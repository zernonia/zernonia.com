...
const { data, pending, error } = useLazyAsyncData("project", () =>
      ...
      <div v-else>
        ...
        >
          ...
          <div class="mt-4 md:mt-0 md:w-2/5 md:ml-12 w-full">
            <h4 class="text-2xl md:text-4xl font-bold font-space mb-2 md:mb-4">
              {{ project.name }}
            </h4>
            ...
            <a class="button-next group" :href="project.link" target="_blank"
              >Visit
              <div
                class="i-feather-external-link w-5 md:w-6 h-5 md:h-6 mb-0.5 duration transition-all ml-2 md:ml-4 group-hover:ml-5"
              />
            </a>
          </div>
        </div>
